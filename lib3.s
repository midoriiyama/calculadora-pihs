...
ler_numero:
    ...
    mov %rsp, %rbp
    and $-16, %rsp
    sub $32, %rsp                  # Espaço seguro na pilha

    # Argumentos do sscanf:
    mov %r12, %rdi                 # 1º Arg: De onde ler? (o nosso ponteiro %r12)
    lea fmt_double_n(%rip), %rsi   # 2º Arg: Formato
    lea 16(%rsp), %rdx             # 3º Arg: Onde salvar o double
    lea 24(%rsp), %rcx             # 4º Arg: Onde salvar a contagem do %n

    xor %rax, %rax                 # Zerar %rax é OBRIGATÓRIO para a libc
    call sscanf                    # ATENÇÃO: Tem que ser sscanf (com dois "S")!

    # Verifica se encontrou um número
    cmp $1, %rax
    jl .erro_leitura

    movsd 16(%rsp), %xmm0          # Coloca o double lido no xmm0
    
    # A MÁGICA: Pula o %r12 passado o número!
    movslq 24(%rsp), %rax          # Pega quantos caracteres o número usou
    addq %rax, %r12                # Empurra o %r12 para frente!
